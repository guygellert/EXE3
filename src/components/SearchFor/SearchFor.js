
import classes from './SearchFor.module.css';
import { useRef,useState,useContext } from 'react';
import AuthContext from '../../store/auth-context';
import { useHistory } from 'react-router-dom';
const SearchFor = () => {
  const enteredID = useRef();
  let ResultName= ""
  const authCtx = useContext (AuthContext); 
  const history = useHistory();
  const [formField,setFormField] = useState({
    searchId:"",
    searchName:"",
    searchPhone:"",
    searchAdress:"",
    searchEmail:""
  })
  if(authCtx.isLoggedin === false){
    history.replace("/")
  }
  const submitHandler = async event => { 
    // var database = firebase.database();
    event.preventDefault(); 

    const searchedID = enteredID.current.value;
    // const enteredNewPassword = newPasswordInputRef.current.value;

    //add validation
    console.log(typeof searchedID)
    let result = await fetch (`https://firestore.googleapis.com/v1/projects/exe-3-23903/databases/(default)/documents/users/1/user?key=AIzaSyCOvCCSMaTTmfb4OXcm6P_LmTe6jb0PaOY`, 
    {
      method: 'GET',
      headers: {
        // 'Authorization': 'AIzaSyCOvCCSMaTTmfb4OXcm6P_LmTe6jb0PaOY',
        'content-type': 'application/json'
      }
    }
    ).then((res) => {
      return res.json();
    })
    ResultName = result.documents.filter((doc)=>{ return doc.fields.ID != null && doc.fields.ID.stringValue === searchedID})[0];
    formField.searchId = (ResultName.fields.ID.stringValue != null)?ResultName.fields.ID.stringValue:"לא צוין";
    formField.searchName = (ResultName.fields.name.stringValue != null)?ResultName.fields.name.stringValue:"לא צוין";
    formField.searchPhone = (ResultName.fields.phone.stringValue != null)?ResultName.fields.phone.stringValue:"לא צוין";
    formField.searchAdress = (ResultName.fields.adress.stringValue != null)?ResultName.fields.adress.stringValue:"לא צוין";
    formField.searchEmail = (
                             ResultName.fields.email !== undefined &&
                             ResultName.fields.email.stringValue !== undefined &&
                             ResultName.fields.email.stringValue !== null)?ResultName.fields.email.stringValue:"לא צוין";
    // console.log(formField.searchName)
    setFormField({...formField})
  }
  return (
    <form className={classes.form} onSubmit ={submitHandler}>
    <section className={classes.profile}>
      <h1>Find By ID</h1>
      <div className={classes.control}>
        <label htmlFor='ID'>ID:</label>
        <input required type='text' id='enterdId' ref={enteredID}/>
      </div>
      <div className={classes.action}>
        <button>Find Profile</button>
      </div>
      <div className={classes.detailsSection}>
      <div className={classes.detailCard}>
        <img src="data:image/jpeg;base64," alt="Avatar" />
        <div className={classes.container}>
          <div>
            <label>ID:</label>
            <input disabled value={formField.searchId}></input>
          </div>
          <div>
            <label>Name:</label>
            <input disabled value={formField.searchName}></input>
          </div>
          <div>
            <label>Phone:</label>
            <input disabled value={formField.searchPhone}></input>
          </div>
          <div>
            <label>Adress:</label>
            <input disabled value={formField.searchAdress}></input>
          </div>
          <div>
            <label>Email:</label>
            <input disabled value={formField.searchEmail}></input>
          </div>
      </div>
    </div>
    </div>
    </section>

    </form>
  );
};

export default SearchFor;
